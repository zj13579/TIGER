...
class FeaturesConstruction:
    def __init__(
        ...
        long_user_refine: bool = False,
        long_user_thresh: int = 400,
        user_kmeans_max_k: int = 7,
    ):
    ...
    def calculate_features(self, similarity_threshold: float):
        ...
        order = np.argsort(arr_u, kind='mergesort')
        ...
        np.add.at(counts, (arr_i, rbin), 1)

        item_total = counts.sum(axis=1)
        ...
        for s, e in zip(idx[:-1], idx[1:]):
            ...
            # —— 6.1 偏离度 & 评分重要性 ——
            r_bar = item_mean[item_ids]                      
            dev = np.abs(ratings - r_bar)                    
            ...
            if L < 2:
                ...
                liked_vecs = iv.index_select(0, liked_ids_t)
